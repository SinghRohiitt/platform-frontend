...
import { Bell, Mail } from "lucide-react"; // better icons

export const Header = () => {
  const { user } = useSelector((state: RootState) => state.auth);

  return (
    <header className="h-20 flex items-center justify-between px-6 bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-20 shadow-sm">
      
      {/* Left Section (Optional Title / Breadcrumbs) */}
